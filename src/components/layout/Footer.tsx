...
const FALLBACK = {
  ...
  tagline:       "A living catalog of products made for real life.",
  copyrightText: "All rights reserved.",
  columns: [
    {
      heading: "Shop",
      links: [
        { label: "All Products",  href: "/shop",              openInNewTab: false },
        { label: "Categories",    href: "/categories",        openInNewTab: false },
        { label: "New Arrivals",  href: "/shop?sort=new",     openInNewTab: false },
      ],
    },
    {
      heading: "Company",
...
async function getFooterData() {
  try {
    ...
    return {
      ...
      columns:       (data.columns      && data.columns.length > 0)      ? data.columns      : FALLBACK.columns,
      socials:       (data.socials      && data.socials.length > 0)      ? data.socials      : FALLBACK.socials,
      bottomLinks:   (data.bottomLinks  && data.bottomLinks.length > 0)  ? data.bottomLinks  : FALLBACK.bottomLinks,
    };
  } catch {
    return FALLBACK;
  }
            ...
            <div className="flex items-center gap-6">
              {(bottomLinks as any[]).map((link, i) => (
                <Link
                  key={i}
                  href={link.href}
                  className="text-xs font-body text-white/25 hover:text-white/50 transition-colors"
                >
                  {link.label}
                </Link>
              ))}
